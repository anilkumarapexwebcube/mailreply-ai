...
const defaultInstruction = document.getElementById(
  "default-instruction",
...
function check() {
  ...
  chrome.runtime.sendMessage({ type: "MAILREPLY_STATUS" }, (response) => {
    if (!response || !response.ok) {
      setState(
        state,
        (response && response.data && response.data.error) || "Not paired yet.",
        "bad",
      );
      return;
    }
    if (!response.data.gmailConnected) {
      setState(
        state,
        "Paired, but Gmail is not connected. Open the dashboard to connect Gmail.",
        "bad",
      );
      return;
    }
    setState(
      state,
      `Ready — Gmail connected as ${response.data.email || "your account"}.`,
      "ok",
    );
  });
}
...
document
  ...
  .addEventListener("click", async () => {
    settings = {
      ...settings,
      platforms: {
        gmail: enableGmail.checked,
        whatsapp: enableWhatsApp.checked,
      },
    ...
    setState(
      saveState,
      "Settings saved. Reload the Gmail/WhatsApp tab to apply.",
      "ok",
    );
  });
