...
export interface GenerateReplyArgs {
  ...
  conversation?: any; // ConversationContext from WhatsApp extension
...
function buildPrompts(args: GenerateReplyArgs): { system: string; prompt: string } {
  ...
  // ── WhatsApp Mode ── (IRON-CLAD: never generate email-style content)
  if (isWhatsApp) {
    const system = [
      ...
      "1. Output ONLY the raw reply text. Nothing else. No preamble like 'Here is your reply:'.",
      "2. NEVER start with 'Dear', 'Hello Sir', 'Hi [Name],' or any formal greeting.",
      "3. NEVER end with 'Regards', 'Best regards', 'Sincerely', 'Thanks and regards', or ANY sign-off.",
      "4. NEVER write your own name, the user's name, or any email address at the end.",
      "5. Do NOT use formal email language. Write exactly like a person texting on WhatsApp.",
      "6. Keep it short and natural. Match the language (Hindi/English/mix) of the conversation.",
      "7. If the conversation is in Hindi, reply in Hindi. If in English, reply in English.",
      "",
      `Tone: ${args.tone}. Length: ${LENGTH_GUIDE[args.length]}`,
    ].join("\n");

    // Format messages from conversation context
    let conversationContext = "";
    if (args.conversation?.messages?.length > 0) {
      conversationContext = args.conversation.messages
        .map((m: any) =>
          `${m.sender?.displayName || (m.direction === "outgoing" ? "You" : "Them")}: ${m.text}`
        )
        .join("\n");
    }
    ...
    const prompt = [
      conversationContext
        ? `WHATSAPP CONVERSATION (read carefully, then reply to the LAST message):\n${conversationContext}`
      ...
      "",
      "YOUR REPLY (plain text only — no greeting, no sign-off, no name at the end):",
    ].join("\n");

    return { system, prompt };
  }

  // ── Gmail Compose mode: no thread context, write a fresh email ──
  ...
  // ── Gmail Reply mode: thread context provided ──
  const system = [
