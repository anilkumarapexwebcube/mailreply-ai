...
export async function startGmailConnectImpl(
  userId: string,
  requestUrl: string,
) {
...
export async function completeGmailConnectionImpl(
  userId: string,
  code: string,
  requestUrl: string,
) {
  ...
  try {
    ...
    if (email)
      await setConnectionAccountEmail(userId, GMAIL_CONNECTOR_ID, email);
  } catch {
